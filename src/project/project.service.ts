...
import { workerListOutputDto, projectOutputDto, workListOutputDto, projectListOutputDto, workOutputDto, markerInfoDto, workerOutputDto, recordOutputDto, workerPositionListDto, ecofieldListOutputDto, ecofieldOutputDto } from './dto/project.output.dto';
...
export class ProjectService {
  ...
  async getEcoListUser(id: number): Promise<ecofieldListOutputDto> {
    ...
    try {
      ...
      const project = await this.worker_role.query(
        `SELECT distinct project_id FROM public.worker_role where worker_id = ` + id
      )
